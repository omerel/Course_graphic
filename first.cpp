...
int sw_top =HEIGHT-60 , sw_bottom = HEIGHT-40,sw = HEIGHT-50;
...
void ShowAll()
{
	double pitch = sw - 510 - 40; // Range of sw is [-40, 40]
...
void display()
{
	...
	glColor4d(0,0,0,1);
	glBegin(GL_POLYGON);
		glVertex2d(-0.2,(HEIGHT-sw)/50.0 -1 -0.1);
		glVertex2d(-0.2,(HEIGHT-sw)/50.0 -1 +0.1);
		glVertex2d(0.2,(HEIGHT-sw)/50.0 -1  +0.1);
		glVertex2d(0.2,(HEIGHT-sw)/50.0  -1 -0.1);
	glEnd();
...
{
	if(x>40 && x<60 && y>sw_top && y<sw_bottom &&
		y < sw_bottom && y < HEIGHT-10 && y > HEIGHT-90)
	{
		sw = y;
		sw_top = sw -10;
		sw_bottom = sw +10;
